...
`public/card.png` (1200x630) is the image a shared link shows; `src/layouts/Layout.astro`'s
head points at it, with `title`/`description` passed in as props from each
page. Replace the image and update the `description` prop on every page. The
card URL resolves against the page that names it, like any link --- `./card.png`
